...
DEFAULT_deploy_lock_file="/run/shanios-deploy.lock"
DEFAULT_deploy_reboot_needed_file="/run/shanios/reboot-needed"
DEFAULT_deploy_channel_file="/etc/shani-channel"
DEFAULT_deploy_license_file="/etc/shani-license"
DEFAULT_deploy_deploy_pending="/data/deployment_pending"
DEFAULT_deploy_boot_failure_file="/data/boot_failure"
DEFAULT_deploy_boot_hard_failure_file="/data/boot_hard_failure"
...
LOG_FILE="${deploy_log_file:-${DEFAULT_deploy_log_file}}"
LOCK_FILE="${deploy_lock_file:-${DEFAULT_deploy_lock_file}}"
REBOOT_NEEDED_FILE="${deploy_reboot_needed_file:-${DEFAULT_deploy_reboot_needed_file}}"
CHANNEL_FILE="${deploy_channel_file:-${DEFAULT_deploy_channel_file}}"
LICENSE_FILE="${deploy_license_file:-${DEFAULT_deploy_license_file}}"
DEPLOY_PENDING="${deploy_deploy_pending:-${DEFAULT_deploy_deploy_pending}}"
BOOT_FAILURE_FILE="${deploy_boot_failure_file:-${DEFAULT_deploy_boot_failure_file}}"
BOOT_HARD_FAILURE_FILE="${deploy_boot_hard_failure_file:-${DEFAULT_deploy_boot_hard_failure_file}}"
...
# NOTE: restore_candidate ERR trap is armed inside deploy_update() immediately
# after the backup snapshot succeeds — the first point where CANDIDATE_SLOT and
# BACKUP_NAME are both set and there is something real to roll back.
...
rollback_system() {
    ...
    if [[ -z "$BACKUP_NAME" ]]; then
        ...
        # a fully clean state (shani-update does this in _handle_fallback_boot,
        # but --rollback can be called independently of shani-update).
        rm -f "$BOOT_FAILURE_FILE" "${BOOT_FAILURE_FILE}.acked" "$BOOT_HARD_FAILURE_FILE" 2>/dev/null || true
...
set_channel_persistent() {
    local chan="${1:-}"
    case "$chan" in
        stable|latest) ;;
        *) die "Invalid channel '${chan}' — must be 'stable' or 'latest'" ;;
    esac

    local current=""
    [[ -f "$CHANNEL_FILE" ]] && current=$(cat "$CHANNEL_FILE" 2>/dev/null | tr -d '[:space:]')

    if [[ "$current" == "$chan" ]]; then
        log "Channel already set to '${chan}' — no change"
        return 0
    fi
